<template>
    <div class="mod-menu">
        <el-form :inline="true" :model="dataForm">
            <el-form-item>
                <el-button v-if="isAuth('sys:sysdept:save')" type="primary"
                    @click="addOrUpdateHandle()">{{ $t("modules.sys.sysdept.form-item-btn") }}</el-button>
            </el-form-item>
        </el-form>
        <el-table :data="dataList" row-key="deptId" border default-expand-all style="width: 100%; ">
            <el-table-column prop="name" header-align="center" min-width="40%"
                :label="$t('modules.sys.sysdept.table-column-1')">
            </el-table-column>
            <el-table-column prop="parentName" header-align="center" align="center" min-width="10%"
                :label="$t('modules.sys.sysdept.table-column-2')">
            </el-table-column>
            <el-table-column prop="orderNum" header-align="center" align="center" min-width="10%"
                :label="$t('modules.sys.sysdept.table-column-3')">
            </el-table-column>
            <el-table-column fixed="right" header-align="center" align="center" width="180"
                :label="$t('modules.sys.sysdept.table-column-4')">
                <template #default="scope">
                    <el-button v-if="isAuth('sys:sysdept:update')" type="text" size="small"
                        @click="addOrUpdateHandle(scope.row.deptId)">{{ $t("modules.sys.sysdept.table-column-4-update") }}</el-button>
                    <el-button v-if="isAuth('sys:sysdept:delete')" type="text" size="small"
                        @click="deleteHandle(scope.row.deptId, scope.row.name)">{{ $t("modules.sys.sysdept.table-column-4-delete") }}</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!-- 弹窗, 新增 / 修改 -->
...
const getDataList = () => {
    ...
    baseService.get('/sys/sysdept/list')
        .then((res) => {
...
const deleteHandle = (id : string , name : string) => {
        ...
        ElMessageBox.confirm(message , titile , {
            ...
            baseService.post(`/sys/sysdept/delete/${id}`)
            .then((res) => {
