<template>
  ...
  <div class="basic member">
    ...
    <businessDictionaryAdd :visible="addVisible" @closeDialog="closeDialog"></businessDictionaryAdd>
    <businessDictionaryUpdate ref="internationalRef" @refreshList="getTableList"></businessDictionaryUpdate>
  </div>
</template>

<script setup lang="ts">
import { deleteBusinessLanguageData, getBusinessLanguageList } from '@/service/business/language';
import { getSysLangageType } from '@/service/sys/language';
import { useAppStore } from '@/store';
import { ElMessage, ElMessageBox } from 'element-plus';
import { onBeforeMount, ref } from 'vue';
import { useI18n } from 'vue-i18n';
import businessDictionaryAdd from './business-dictionary-add.vue';
import businessDictionaryUpdate from './business-dictionary-update.vue';
...
const getTableList = async (page?: number) => {
  ...
  // 请求
  const data = await getBusinessLanguageList(params);
  // 判断
...
const deleteRow = (row: any, index: number) => {
  ...
  let message = t('modules.multi-dictionary.business.business-dictionary-list.delete-confirm', { indexNum: indexNum });
  let title = t('modules.multi-dictionary.business.business-dictionary-list.delete-title');
  let confirm = t('modules.multi-dictionary.business.business-dictionary-list.confirmButtonText');
  let cancel = t('modules.multi-dictionary.business.business-dictionary-list.cancelButtonText');
  let success = t('modules.multi-dictionary.business.business-dictionary-list.delete-success');
  ...
  ElMessageBox.confirm(message, title, {
    ...
    // 获取数据
    const data = await deleteBusinessLanguageData({
