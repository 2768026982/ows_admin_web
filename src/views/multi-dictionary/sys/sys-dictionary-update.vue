<template>
  <!--共通國際化修改頁-->
  <div>
    <el-dialog :title="$t('modules.multi-dictionary.sys.sys-international.dialog-title')" @closed="closeFinish"
      ...
      <div class="admin-info">
        <el-form ref="updateFormRef" :label-position="'right'" v-loading="dataLoading" :model="dataForm" :rules="rules"
          ...
          <el-row type="flex" justify="space-between">
            <el-col :span="8">
              <el-form-item :label="$t('modules.multi-dictionary.sys.sys-international.form-item-1')" prop="itemKey">
                <el-input v-model="dataForm.itemKey" disabled></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item :label="$t('modules.multi-dictionary.sys.sys-international.form-item-2')"
                prop="functionCode">
                <el-input v-model="dataForm.functionCode" disabled></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item :label="$t('modules.multi-dictionary.sys.sys-international.form-item-3')" prop="itemType">
          ...
          <el-row type="flex" justify="space-around">
            <el-col :span="8">
              <el-form-item :label="$t('modules.multi-dictionary.sys.sys-international.form-item-4')" prop="languageId"
                ref="languageId">
            ...
            <el-col :span="10">
              <el-form-item :label="$t('modules.multi-dictionary.sys.sys-international.form-item-5')" prop="content">
          ...
          <el-row type="flex" justify="space-around">
            ...
            <el-col :span="8" class="d-flex flex-end">
              <el-button type="primary" @click="save" v-re-click>{{
                $t('modules.multi-dictionary.sys.sys-international.save-btn') }}</el-button>
            </el-col>
          </el-row>
        </el-form>
          ...
          <el-col :span="24">
            <el-table :data="tableList" border v-loading="dataListLoading" style="width: 100%">
              <el-table-column header-align="center" align="center" fixed="left" type="index" width="60"
                :label="$t('modules.multi-dictionary.sys.sys-international.table-column-1')">
                <template v-slot="scope">
                  {{ scope.$index + 1 }}
                </template>
              </el-table-column>
              <el-table-column align="center"
                :label="$t('modules.multi-dictionary.sys.sys-international.table-column-2')" min-width="15%">
                <template v-slot="scope">
                  <div v-if="scope.row.primeLanguage == '1'">
                    {{ $t('modules.multi-dictionary.sys.sys-international.table-column-2-yes') }}</div>
                  <div v-else>{{ $t('modules.multi-dictionary.sys.sys-international.table-column-2-no') }}</div>
                </template>
              </el-table-column>
              <el-table-column align="center"
                :label="$t('modules.multi-dictionary.sys.sys-international.table-column-3')" min-width="15%">
                <template v-slot="scope">
                  {{ global_variable.filterObj(langList, 'langTypeId', scope.row.langTypeId).langTypeName }}
                </template>
              </el-table-column>
              <el-table-column align="center"
                :label="$t('modules.multi-dictionary.sys.sys-international.table-column-4')" min-width="40%">
                <template v-slot="scope">
                  <el-input type="textarea" resize="none" :autosize="{ minRows: 2, maxRows: 4 }"
                    v-model="scope.row.languageContent"></el-input>
                </template>
              </el-table-column>
              <el-table-column :label="$t('modules.multi-dictionary.sys.sys-international.table-column-5')" width="200"
                fixed="right" align="center">
                <template v-slot="scope">
                  <el-button plain type="primary" link @click="updateData(scope.row)">
                    {{ $t('modules.multi-dictionary.sys.sys-international.table-column-5-update') }}
                  </el-button>
                  <!--是主要語言不讓刪除  primeLanguage：1是主要語言 0不是主要語言-->
                  <el-button plain type="danger" :disabled="scope.row.primeLanguage == '1'" link
                    @click="deleteRow(scope.row, scope.$index)">
                    {{ $t('modules.multi-dictionary.sys.sys-international.table-column-5-delete') }}
                  </el-button>
                </template>
              </el-table-column>
            </el-table>
...
import { getSysLangageType, getSysLanguageContent, getSysLanguageDelete, getSysLanguageInsert, getSysLanguageUpdate } from '@/service/sys/language';
...
let rules = ref({
  languageId: [
    { required: true, message: t('modules.multi-dictionary.sys.sys-international.rules-languageId'), trigger: 'change' },
  ]
});
// 表格数据列表
let tableList = ref([]);
...
const getTableList = async (page?: number) => {
  ...
  // 请求
  const data = await getSysLanguageContent({
    //功能代码
...
const save = () => {
  updateFormRef.value.validate(async (valid: boolean) => {
    if (valid) {
      const data = await getSysLanguageInsert({
        //功能代碼
        functionCode: dataForm.value.functionCode,
        //項目類型
        itemType: dataForm.value.itemType,
...
const updateData = async (row: any) => {
  // 请求
  const data = await getSysLanguageUpdate({
    //词汇语系种类ID
...
const deleteRow = (row: any, index: number) => {
  ...
  ElMessageBox.confirm(message, title, {
    ...
    // 获取数据
    const data = await getSysLanguageDelete({
      //词汇语系种类ID
      langTypeId: row.langTypeId,
      //词汇ID
      pcItemId: row.pcItemId
